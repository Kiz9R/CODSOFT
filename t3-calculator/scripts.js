        ...
        if (key.innerText === "←") {
          key.addEventListener("click", () => {
            dispResult.textContent =
              ...
              (dispResult.textContent.length === 2 &&
                dispResult.textContent[0] === "-")
                ? "0"
                : dispResult.textContent.replace(/.$/, "");
...
function AppendOperator(ope) {
  ...
  if (switchOperator) {
    let regex = new RegExp(
      `(.*)([${Array.from(opMap.values()).join(",")}])(.*)$`,
      "gi"
    );
    dispCurrCalc.textContent = dispCurrCalc.textContent.replace(
      regex,
      `$1${opMap.get(ope)}$3`
    );
    ...
    dispCurrCalc.textContent += `${checkForBrackets(
      dispResult.textContent
    )} ${opMap.get(ope)} `;
    dispResult.textContent = "0";
    switchOperator = true;
...
function Calcul() {
  if (
    ...
    dispResult.textContent.slice(-1) === "."
  )
    return;
  let calc = `${dispCurrCalc.textContent}${checkForBrackets(
    dispResult.textContent
  )}`;
  if (calc.slice(-3) === "/ 0") {
